...
class Detect(nn.Module):
    ...
    def __init__(self, nc=80, anchors=(), ch=(), inplace=True):  # detection layer
        ...
        #即把9个anchor分开了，分成三层，每层三个anchor，每个anchor是二维
        #模型中需要保存下来的参数包括两种：一种是反向传播需要被optimizer更新的，称之为parameter
        #另一种是反向传播不更新的，称之为buffer
        #对于buffer，我们需要创建tensor，再通过register_buffer()进行注册
        #然后可以通过module.buffer()返回，注册完后参数也会自动保存到OrderDict中
        ...
        #其中ch是个list:3 [128,256,512]是得到的结果的通道的取值
...
class Decoupled_Detect(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            ...
            x[i] = torch.cat(
                [self.reg_head(reg_heads), self.obj_head(reg_heads), self.cls_head(self.cls_c2(self.cls_c1(x[i])))], 1)
...
class Model(nn.Module):
    def __init__(self, cfg='yolov5s.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        ...
        m = self.model[-1]  # Detect()，把detect复制给m
        if isinstance(m, Detect)or isinstance(m,Decoupled_Detect):
            s = 256  # 2x min stride
            m.inplace = self.inplace
            m.stride = torch.tensor([s / x.shape[-2] for x in self.forward(torch.zeros(1, ch, s, s))])  # forward
            #m.stride = [8,16,32]，三个层的stride，相当于下采样倍数
            m.anchors /= m.stride.view(-1, 1, 1)
            #anchor大小计算，如 [10,13]变成[1.25,1.625]
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    #把模型结构的depth_multiple赋值给gd，width_multiple赋值给gw
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        #f：from，n：number，m：module，args：args
        ...
        if m in [Conv, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, MixConv2d, Focus, CrossConv,
                 BottleneckCSP, C3, C3TR, C3STR, C3SPP, C3Ghost, TransformerBlock,CBAM, CoordAtt,Shuffle_CoA,ShuffleV2Block_lite, ShuffleV2Block,FeatureSelectionModule, FeatureAlign_V2,SNLUnit,ECAC3,CoAC3]:
            c1, c2 = ch[f], args[0]
            #控制宽度的代码
            ...
            args = [c1, c2, *args[1:]]
            if m in [BottleneckCSP, C3, C3TR, C3STR, C3Ghost,TransformerBlock, CBAM,ECAC3,CoAC3]:
            ...
            c2 = ch[f]
        #*args表示接收任意个数量的的参数，调用时把实际参数打包为一个元组传入实参
        #相当于网络架构图中的n×Bottleneck

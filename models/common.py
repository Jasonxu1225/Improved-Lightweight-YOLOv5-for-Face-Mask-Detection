...
from utils.general import (colorstr, increment_path, make_divisible, non_max_suppression, scale_coords,
                           xyxy2xywh)
...
class Conv(nn.Module):
    # Standard convolution，包括：conv+BN+hardswish。相当于网络架构图中的蓝色Conv
...
class DWConv(Conv):
    # Depth-wise convolution class，好像没有用到
...
class Bottleneck(nn.Module):
    # Standard bottleneck，相当于网络架构图中的橙色和黄色的Bottleneck模块，其中True/False是shortcut值
    ...
    def forward(self, x):
        #根据self.add的值确定是否要做add
        return x + self.cv2(self.cv1(x)) if self.add else self.cv2(self.cv1(x))


class BottleneckCSP(nn.Module):
    # CSP Bottleneck https://github.com/WongKinYiu/CrossStagePartialNetworks
    #相当于网络架构图中的橙色和黄色的BCSPn模块，其中True/False是shortcut值
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        self.act = nn.SiLU()
        #*操作符可以把一个list拆开成一个个独立的元素，相当于n次bottleneck
...
class FeatureAlign_V2(nn.Module):  # FaPN full version
    def __init__(self, in_nc=128, out_nc=128, norm=None):
        ...
        self.c3 = C3(out_nc, out_nc,n=1,shortcut=True)
    ...
    def forward(self, feat_l, feat_s):
        ...
        feat_align = self.relu(self.c3([feat_up, offset]))  # [feat, offset]
...
class CoordAtt(nn.Module):
    ...
    def forward(self, x):
        ...
        return out
class CoAC3(nn.Module):
    # CSP Bottleneck with 3 convolutions
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        super(CoAC3, self).__init__()
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = Conv(c1, c_, 1, 1)
        self.cv2 = Conv(c1, c_, 1, 1)
        self.cv3 = Conv(2 * c_, c2, 1)  # act=FReLU(c2)
        self.m = nn.Sequential(*[Bottleneck(c_, c_, shortcut, g, e=1.0) for _ in range(n)])
        self.coa_layer = CoordAtt(c1,c2)
        # self.m = nn.Sequential(*[CrossConv(c_, c_, 3, 1, g, 1.0, shortcut) for _ in range(n)])

    def forward(self, x):
        return self.coa_layer(self.cv3(torch.cat((self.m(self.cv1(x)), self.cv2(x)), dim=1)))


class Shuffle_CoA(nn.Module):
    def __init__(self, inp, oup, stride):
        super(Shuffle_CoA, self).__init__()
        ...
        self.coa_layer = CoordAtt(inp, oup)
...
class ShuffleV2Block_lite(nn.Module):
    def __init__(self, inp, oup, stride):
        super(ShuffleV2Block_lite, self).__init__()
...
class Focus(nn.Module):
    ...
    #把宽度w和高度h的信息整合到c空间中，通过分成四个slice
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True):  # ch_in, ch_out, kernel, stride, padding, groups
        super().__init__()
